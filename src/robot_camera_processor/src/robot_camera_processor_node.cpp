...
public:
    RobotCameraProcessor() : Node("robot_camera_processor") {
        ...
        // Матрица камеры (после калибровки)
        camera_matrix_ = (cv::Mat_<double>(3, 3) <<
                              545.76399507, 0.0, 311.28402447,
                          0.0, 545.29751391, 253.39129559,
                          0.0, 0.0, 1.0);

        // Коэффициенты дисторсии (после калибровки)
        dist_coeffs_ = (cv::Mat_<double>(1, 5) <<
                            -0.169775500, 0.438176287, 0.000680997688, -0.000881722448, -1.01195657);
...
// class RobotCameraProcessor : public rclcpp::Node {
// public:
//     RobotCameraProcessor() : Node("robot_camera_processor"), camera_index_(2) {// index 2 - дял камеры с USB, 0 - для встроенной
